defmodule Vaultex.Auth do
  ...
  def handle(:userpass, {username, password}, state) do
    request(:post, "#{state.url}auth/userpass/login/#{username}", %{password: password}, [
      {"Content-Type", "application/json"}
    ])
    |> handle_response(state)
  end
  ...
  def handle(:ec2, {role}, state) do
    ...
    request(:post, "#{state.url}auth/aws-ec2/login", %{pkcs7: pkcs7, nonce: nonce, role: role}, [
      {"Content-Type", "application/json"}
    ])
    |> handle_response(state)
  end
  ...
  defp handle_response({:ok, response}, state) do
    case response.body |> Poison.Parser.parse!() do
      ...
      %{"auth" => auth, "data" => nil} ->
        {:reply, {:ok, :authenticated}, Map.merge(state, %{token: auth["client_token"]})}
    end
  end
